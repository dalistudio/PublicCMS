...
( function($) {
	...
	$.fn.jBar = function(options) {
		var op = $.extend({
			container : "#navTab" , collapse : ".collapse" , toggleBut : ".toggleCollapse div" , sideBar : "#sidebar" , sideBar2 : "#sidebar_s" , splitBar : "#splitBar" ,
...
( function($) {
	...
	$.extend($.dwz.accordion, {
		defaults : {
			selectedClass : "collapsable" , alwaysOpen : true , animated : 'slide' , event : "click" , header : ".accordionHeader" , autoheight : true , running : 0 ,
...
function initLayout() {
	...
	$("#sidebar #menu").height(iContentH - $('#sidebar .toggleCollapse').height());
	$("#taskbar").css({
...
function initUI(_box) {
	...
	$(".searchBar .searchContent", $p).click(function(){
	...
	$('div.accordion', $p).each(function() {
		var $this = $(this);
		$this.accordion({
			fillSpace : "sidebar" , alwaysOpen : false , active : 0
		});
	});
	...
	if ($("textarea.editor", $p).length > 0 ) {
		...
		$("textarea.editor", $p).each(function(i) {
			...
			if ($this.attr('maxlength') )
				ueditor_loader[module][i].setOpt({
...
var navTab = {
	...
	_reload : function($tab, flag) {
		...
		if (navMenuIndex && navMenuIndex != $('#navMenu>li.selected').index() )
			$('#navMenu>li').eq(navMenuIndex).find('a').click();
	...
	openTab : function(tabid, url, options) { // if found tabid replace tab,
		...
		if (iOpenIndex >= 0 ) {
			...
			this._tabBox.append('<li tabid="' + tabid + '"><a href="javascript:" title="' + op.title + '">' + op.titleHtml
					+ '</a><a href="javascript:;" class="close"><i class="icon-remove"></i></a></li>');
			this._panelBox.append('<div class="page unitBox"></div>');
			this._moreBox.append('<li><a href="javascript:" title="' + op.title + '">' + op.titleHtml + '</a></li>');
			var $tabs = this._getTabs();
		...
		this._init();
		this._scrollCurrent();
		this._getTabs().eq(this._currentIndex).attr("url", url).attr("navMenuIndex", $('#navMenu>li.selected').index());
	}
};
/**
 * @author ZhangHuihua@msn.com
...
( function($) {
	$.pdialog = {
		_op : {
			height : 500 , width : 800 , minH : 40 , minW : 50 , total : 20 , max : false , mask : false , resizable : true , drawable : true , maxable : true , minable : true ,
			fresh : true
		} ,
		_current : null ,
		_zIndex : 42 ,
